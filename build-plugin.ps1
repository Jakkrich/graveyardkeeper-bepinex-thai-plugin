...
$constants = 'namespace GK2Thai.Plugin.Runtime { internal static class BuildConstants { internal const string FontHash = "' + $hashes['font.ttf'] + '"; internal const string PayloadHash = "' + $hashes['payload.bin'] + '"; internal const string GlyphHash = "' + $hashes['glyphs.png'] + '"; } }'
...
$dll = Join-Path $build 'GK2Thai.Plugin.dll'
$argsCsc = @('/nologo','/noconfig','/nostdlib+','/target:library',"/out:$dll") + @($refs | ForEach-Object { '/reference:' + $_ })
$argsCsc += @('payload.bin','glyphs.png','metadata.json') | ForEach-Object { '/resource:' + (Join-Path $payload $_) + ',GK1Thai.' + $_ }
$argsCsc += @(Get-ChildItem -LiteralPath (Join-Path $PSScriptRoot 'GK2Thai.Plugin') -Recurse -Filter '*.cs' | ForEach-Object FullName)
...
$inputs = @{}
$watched = @($refs) + @(Get-ChildItem -LiteralPath (Join-Path $PSScriptRoot 'GK2Thai.Plugin') -Recurse -Filter '*.cs' | ForEach-Object FullName) + @($PSCommandPath)
...
$outputs = @{}
foreach ($name in @('GK2Thai.Plugin.dll','font.ttf')) { $outputs[$name] = (Get-FileHash -LiteralPath (Join-Path $build $name) -Algorithm SHA256).Hash.ToLowerInvariant() }
